# Ben 10 AI Assistant — RAG, Structured-Data & Agentic AI

> An open-source reference AI assistant combining vector retrieval over multi-format
> documents, a natural-language-to-SQL pipeline over a PostgreSQL database, and an
> agentic ReAct loop — featuring **Ben 10 (Ben Tennyson)** as Raghul's personal AI companion
> for **mental well-being, day-to-day motivation, life simplification, and technical learning**.

Everything runs **open-source** — Groq for the LLM, `sentence-transformers`
for embeddings, Qdrant (embedded) for the vector DB, PostgreSQL for the
structured DB.
...
4. [Complete Setup](#4-complete-setup)
5. [Sample Queries](#5-sample-queries)

---

## 1. Project Overview

This AI assistant serves as **Ben 10 (Ben Tennyson)**, supporting Raghul in four primary areas:

1. **Personal Mental Well-Being**: Empowering, positive, and empathetic hero support to manage stress and build resilience.
2. **Day-to-Day Motivation**: High-energy hero drive ("It's Hero Time!") to defeat procrastination and keep Raghul inspired.
3. **Life Simplification**: Actionable ideas to streamline daily habits, routines, and task management.
4. **Technical & Learning Growth**: Step-by-step guidance in Machine Learning, Generative AI, RAG, Agentic AI, FastAPI, and backend development.

**Two modes, switchable from the sidebar:**
- **RAG mode** — Ben 10 answers questions grounded in the Omnitrix vector knowledge base only.
- **Agentic mode** — Ben 10 uses a `TOOL_REGISTRY` (knowledge retrieval, SQL database queries, memory, reminders, action logs) with a ReAct planning loop (Thought → Action → Observation).
...
### 2.2 Structured fleet-ops DB & the NL2SQL pipeline

The vector KB is great at narrative/procedural knowledge and bad at precise aggregates —
"how many times has the Mark 42 needed left-boot-thruster repairs" needs an exact COUNT,
not an LLM eyeballing a handful of retrieved chunks. So the fleet's operational records
...
readonly_engine.execute()   <- runs as the `jarvis_readonly` Postgres role, which has
    │                          SELECT-only grants on the 4 fleet tables (THE REAL
...
pipeline._synthesize_answer()   <- second LLM call turns the result rows into a
                                    JARVIS-voiced answer, citing "structured fleet records"
```
